...
<c:forEach var="nid" items="${searchNeighborhoodIds}" varStatus="st">
    <c:set var="nbIdsJson" value="${nbIdsJson}${nid}"/>
    <c:if test="${!st.last}"><c:set var="nbIdsJson" value="${nbIdsJson},"/></c:if>
</c:forEach>
<c:set var="nbIdsJson" value="${nbIdsJson}]"/>
