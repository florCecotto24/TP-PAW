...
@Component
public final class UserFavoritesController {
    ...
    public Response listFavorites(
        ...
        final List<CarDto> dtos = favorites.getContent().stream()
    ...
    @PUT
    @Path("/{carId}")
    public Response addFavorite(@PathParam("id") final long id, @PathParam("carId") final long carId) {
        userResourceAccess.requireSelf(id, currentUserResolver.currentPrincipalOrNull());
        userService.getUserById(id)
                .orElseThrow(() -> new UserNotFoundException(MessageKeys.USER_ACCOUNT_NOT_FOUND));
    ...
    @DELETE
    @Path("/{carId}")
    public Response removeFavorite(@PathParam("id") final long id, @PathParam("carId") final long carId) {
        userResourceAccess.requireSelf(id, currentUserResolver.currentPrincipalOrNull());
        userService.getUserById(id)
                .orElseThrow(() -> new UserNotFoundException(MessageKeys.USER_ACCOUNT_NOT_FOUND));
        if (favCarService.isFavorited(carId, id)) {
