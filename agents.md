...
- **models** — Organised by *feature/role* rather than by type:
  ...
  - `pagination/` — `Page` slice (`DualLayerPageWindow`, `SingleLayerPageWindow`) and `UiPaging`. UI page sizes live in `AppPaginationProperties` (webapp); the DB fetch window lives in `DbPaginationConfig` (persistence). Services do not read pagination policy.
  - `security/` — `UserRole`.
