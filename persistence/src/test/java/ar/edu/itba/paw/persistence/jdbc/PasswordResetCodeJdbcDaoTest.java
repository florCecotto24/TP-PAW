...
class PasswordResetCodeJdbcDaoTest extends DaoIntegrationTestSupport {
    ...
    void testInsertPersistsRowWithExpectedColumns() {
        ...
        // 2.Exercise
        dao.insert(USER_ID, "abcdef", expires, now);

        // 3.Assert
        final Map<String, Object> row = jdbcTemplate.queryForMap(
                "SELECT user_id, code, expires_at, created_at FROM password_reset_codes WHERE user_id = ?",
                USER_ID);
        Assertions.assertEquals(USER_ID, ((Number) row.get("USER_ID")).longValue());
        Assertions.assertEquals("abcdef", row.get("CODE"));
    ...
    void testDeleteForUserRemovesAllRowsForThatUser() {
        ...
        jdbcTemplate.update(
                "INSERT INTO password_reset_codes (user_id, code, expires_at, created_at) VALUES (?, ?, ?, ?)",
                2L, "222222", Timestamp.from(expires), Timestamp.from(now));
        ...
        final Long forUser = jdbcTemplate.queryForObject(
                "SELECT COUNT(*) FROM password_reset_codes WHERE user_id = ?", Long.class, USER_ID);
        final Long otherUser = jdbcTemplate.queryForObject(
                "SELECT COUNT(*) FROM password_reset_codes WHERE user_id = ?", Long.class, 2L);
        Assertions.assertEquals(0L, forUser);
        Assertions.assertEquals(1L, otherUser);
    }

    @Test
    void testDeleteIfValidRemovesRowAndReturnsTrueOnExactMatch() {
        // 1.Arrange
        insertUser(USER_ID, "user@example.com", "Ada", "Lovelace");
        final Instant now = Instant.parse("2026-05-03T13:00:00Z");
        jdbcTemplate.update(
    ...
    @Test
    void testDeleteIfValidReturnsFalseWhenWrongCode() {
        // 1.Arrange
        insertUser(USER_ID, "user@example.com", "Ada", "Lovelace");
        final Instant now = Instant.parse("2026-05-03T13:00:00Z");
        jdbcTemplate.update(
                "INSERT INTO password_reset_codes (user_id, code, expires_at, created_at) VALUES (?, ?, ?, ?)",
                USER_ID, "abcdef", Timestamp.from(now.plusSeconds(60)), Timestamp.from(now));

        // 2.Exercise
        final boolean ok = dao.deleteIfValid(USER_ID, "ZZZZZZ", now);
    ...
    @Test
    void testDeleteIfValidReturnsFalseWhenExpired() {
        // 1.Arrange
        insertUser(USER_ID, "user@example.com", "Ada", "Lovelace");
        final Instant now = Instant.parse("2026-05-03T13:00:00Z");
        jdbcTemplate.update(
                "INSERT INTO password_reset_codes (user_id, code, expires_at, created_at) VALUES (?, ?, ?, ?)",
                USER_ID, "abcdef", Timestamp.from(now.minusSeconds(1)), Timestamp.from(now.minusSeconds(60)));

        // 2.Exercise
        final boolean ok = dao.deleteIfValid(USER_ID, "abcdef", now);
    ...
    @Test
    void testHasActiveCodeIsTrueWhenAtLeastOneFreshCodeExists() {
        // 1.Arrange
        insertUser(USER_ID, "user@example.com", "Ada", "Lovelace");
        final Instant now = Instant.parse("2026-05-03T13:00:00Z");
        jdbcTemplate.update(
                "INSERT INTO password_reset_codes (user_id, code, expires_at, created_at) VALUES (?, ?, ?, ?)",
                USER_ID, "stale1", Timestamp.from(now.minusSeconds(1)), Timestamp.from(now.minusSeconds(60)));
        jdbcTemplate.update(
                "INSERT INTO password_reset_codes (user_id, code, expires_at, created_at) VALUES (?, ?, ?, ?)",
                USER_ID, "fresh1", Timestamp.from(now.plusSeconds(1)), Timestamp.from(now));
    ...
    @Test
    void testHasActiveCodeIsFalseWhenAllCodesExpiredOrNoneExist() {
        // 1.Arrange
        insertUser(USER_ID, "user@example.com", "Ada", "Lovelace");
        final Instant now = Instant.parse("2026-05-03T13:00:00Z");
        jdbcTemplate.update(
        ...
        // Same user with all rows removed.
        jdbcTemplate.update("DELETE FROM password_reset_codes WHERE user_id = ?", USER_ID);
        Assertions.assertFalse(dao.hasActiveCode(USER_ID, now));
    }
}
