...
class PasswordResetCodeJpaDaoTest extends DaoIntegrationTestSupport {
    ...
    void seedUser() {
        jdbcTemplate.update(
                ...
                "prc@test.com", "Prc", "User");
        userId = jdbcTemplate.queryForObject(
                "SELECT id FROM users WHERE email = ?", Long.class, "prc@test.com");
    }
    ...
    void testDeleteForUserRemovesAllCodesForUser() {
        ...
        // 3.Assert
        final Long count = jdbcTemplate.queryForObject(
                "SELECT COUNT(*) FROM password_reset_codes WHERE user_id = ?",
                Long.class,
                userId);
        Assertions.assertEquals(0L, count);
    }
