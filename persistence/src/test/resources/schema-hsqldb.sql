...
CREATE SEQUENCE car_availability_id_seq AS BIGINT START WITH 1 INCREMENT BY 1;
...
CREATE SEQUENCE reviews_id_seq AS BIGINT START WITH 1 INCREMENT BY 1;
...
CREATE SEQUENCE email_verification_codes_id_seq AS BIGINT START WITH 1 INCREMENT BY 1;
CREATE SEQUENCE password_reset_codes_id_seq AS BIGINT START WITH 1 INCREMENT BY 1;
...
-- HSQLDB does not support expression indexes (LOWER(name)); tests use exact-case uniqueness.
-- PostgreSQL enforces case-insensitive uniqueness via V45 (LOWER(name)).
CREATE UNIQUE INDEX IF NOT EXISTS car_brands_name_uidx ON car_brands (name);
...
CREATE UNIQUE INDEX IF NOT EXISTS car_models_brand_name_uidx ON car_models (brand_id, name);

ALTER TABLE cars ADD CONSTRAINT fk_cars_model_id FOREIGN KEY (model_id) REFERENCES car_models(id);
CREATE UNIQUE INDEX IF NOT EXISTS cars_owner_id_plate_uidx ON cars (owner_id, plate);
...
CREATE TABLE IF NOT EXISTS car_availability(
    ...
    CONSTRAINT car_availability_kind_check CHECK (kind IN ('offered', 'withdrawn')),
...
CREATE TABLE IF NOT EXISTS reservations (
    ...
    pickup_reminder_email_sent BOOLEAN NOT NULL DEFAULT FALSE,
...
CREATE TABLE IF NOT EXISTS reservations_availabilities (
    ...
    FOREIGN KEY (availability_id) REFERENCES car_availability(id)
);
...
CREATE TABLE IF NOT EXISTS reviews (
    ...
    CONSTRAINT reviews_reservation_side_unique UNIQUE (reservation_id, made_by_rider)
);
...
CREATE TABLE IF NOT EXISTS otp_attempt_windows (
