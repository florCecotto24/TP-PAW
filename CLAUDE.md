...
- **Fixed literals in matchers**: Do not use `Mockito.anyLong()` (or `any*()`) as the value inside `when()`/`verify()` — use fixed literals so tests are deterministic and intention is clear.
