...
/**
 * State-based in-memory fake for {@link PasswordResetCodeDao} (AGENTS.md rule TEST-8): the single
 * active row per user lives in a map, writes mutate it and the read-side contract
 * ({@code hasActiveCode}, {@code matchesValidCode}, {@code deleteIfValid}) answers from it. Tests
 * observe the SUT through this persisted state — {@link #storedCodeFor(long)} reads the row (the
 * SHA-256 digest the service stores) a write left behind — never through recorded interactions.
 */
public class InMemoryPasswordResetCodeDao implements PasswordResetCodeDao {
    ...
    public boolean matchesValidCode(final long userId, final String code, final Instant now) {
        final CodeRow row = rowsByUserId.get(userId);
        return row != null && row.code().equals(code) && row.expiresAt().isAfter(now);
    }
